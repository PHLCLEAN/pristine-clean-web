    ...
    <div className="min-h-screen bg-phl-ground">
      ...
      <main>
            ...
            <ol className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-x-10 gap-y-8">
              {service.process.map((step, i) => (
                <li key={i} className="flex gap-4">
                  <span className="font-mono text-[12px] text-phl-blue pt-1 shrink-0 tabular-nums">{String(i + 1).padStart(2, '0')}</span>
        ...
        <section className="section-padding bg-phl-navy border-b border-phl-rule">
          <div className="container-custom">
            <div className="flex items-baseline gap-4 mb-5 font-mono text-[11px] tracking-[0.14em] uppercase text-phl-blue-soft">Why us<span className="flex-1 h-px bg-white/20" /></div>
            ...
            <Accordion type="single" collapsible className="w-full">
              {service.faqs.map((faq, i) => (
                <AccordionItem key={i} value={`item-${i}`} className="border-phl-rule">
                  <AccordionTrigger className="text-left text-[16px] font-medium text-phl-ink hover:text-phl-blue hover:no-underline">{faq.question}</AccordionTrigger>
                  <AccordionContent className="text-[14px] text-phl-ink-2 leading-relaxed">{faq.answer}</AccordionContent>
                </AccordionItem>
              ))}
            </Accordion>
          </div>
          ...
          <div className="container-custom">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16">
              <div>
                ...
                <h2 className="text-[clamp(1.5rem,3vw,2.1rem)] text-phl-ink">Schedule a walkthrough for {service.shortTitle.toLowerCase()}</h2>
                <p className="mt-4 text-phl-ink-2 leading-relaxed max-w-[50ch]">We won&rsquo;t price a floor we haven&rsquo;t stood on. The walkthrough is free, takes under an hour, and you get an honest read on what will and won&rsquo;t come back whether or not you hire us.</p>
                <div className="mt-8 pt-8 border-t border-phl-rule"><ContactInfo /></div>
              </div>
              <QuoteForm />
            </div>
          </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-phl-rule border border-phl-rule rounded-sm overflow-hidden">
                {relatedPosts.map(post => (
                  <Link key={post.slug} to={`/blog/${post.slug}`} className="group bg-phl-surface hover:bg-phl-raised transition-colors p-5 pb-6">
                    <div className="flex items-center gap-2 font-mono text-[10.5px] uppercase tracking-[0.08em] text-phl-muted mb-3"><Clock className="h-3.5 w-3.5" /><span>{post.readTime}</span></div>
        ...
        <section className="section-padding">
          <div className="container-custom">
            ...
            <div className="flex flex-wrap gap-2.5">
              {locations.map(l => (
                <Link key={l.slug} to={`/services/${service.slug}/${l.slug}`} className="bg-phl-surface border border-phl-rule hover:border-phl-blue hover:text-phl-blue rounded-sm px-4 py-2.5 text-[13.5px] text-phl-ink-2 transition-colors">{service.shortTitle} in {l.shortName}</Link>
              ))}
            </div>
          </div>
        </section>
